...
[![NPM Package](https://img.shields.io/npm/v/ozbcore-lib.svg?style=flat-square)](https://www.npmjs.org/package/ozbcore-lib)
...
The complete docs are hosted here: [ozbcore documentation](http://bitcore.io/guide/). There's also a [ozbcore API reference](http://bitcore.io/api/) available generated from the JSDocs of the project, where you'll find low-level details on each ozbcore utility.
...
To build a ozbcore-lib full bundle for the browser:

```sh
gulp browser
```

This will generate files named `ozbcore-lib.js` and `ozbcore-lib.min.js`.
