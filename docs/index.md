...
To get started, just `npm install ozbcore` or `bower install ozbcore`.
...
var imported = ozbcore.PrivateKey.fromWIF(exported);
...
var p2shAddress = new ozbcore.Address([publicKey1, publicKey2, publicKey3], 2);
```
...
var uri = new ozbcore.URI(paymentInfo).toString();
```
